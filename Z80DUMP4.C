/*
    ...
    version 3.05, and may not be distributed.  You may use this source for
...
char **str;
{
    printf ("Z80DUMP - Z80/SLT file inspection utility - (c) 1999 G.A. Lunter - version 4.00\n");
    ...
    if (ver==3) {
       ...
       if (buffer.viertel>=4)
          printf ("Wrong Format!\n"); else
          printf ("%ld T states after interrupt\n",17472L*(long)(((buffer.viertel+1)&3)+1)-1-(long)buffer.tstates);
    ...
    if (ver<2) {
         ...
         while ((i==3)&&(blockhdr.length)) {
             int toread=blockhdr.length;
             if (toread==-1) toread=16384;
             fgetpos(fin,&pos);
             printf ("#%05lX Page nr: %2d      Block length: %d:  ",pos-3,blockhdr.page,toread);
             i=fread(buf1,1,toread,fin);
             if (i!=toread) {
                 printf ("\n\nError in .Z80 file: partial block (length: %d) found\n",i);
                 return(1);
             }
             if ((int)blockhdr.length==-1)
             ...
             printf ("OK\n");
             if ((int)blockhdr.length>16384) printf ("  (Warning: previous block is larger than 16 Kb in compressed form.)\n");
